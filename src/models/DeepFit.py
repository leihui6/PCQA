...
def fit_Wjet(points, weights, order=2, compute_neighbor_normals=False):
    ...
    # print (f'info: w_vector:{w_vector.shape}, {w_vector}')
    if order > 1:
        #pre conditioning
    ...
    if order == 1:
        ...
        # print (f'info: test0 {torch.cat([h, h, h_2, h_2, h_2, h_3, h_3, h_3, h_3, torch.ones_like(h)], dim=1).shape}')
    ...
    # print (f'info:{A.permute(0, 2, 1).shape},  {(w_vector * A).shape}')
    ...
    # print (f'info beta3:{torch.ones(batch_size, 1, device=x.device, dtype=beta.dtype).shape}')
    n_est = torch.nn.functional.normalize(torch.cat([-beta[:, 0:2].squeeze(-1), torch.ones(batch_size, 1, device=x.device, dtype=beta.dtype)], dim=1), 
    p=2, dim=1)
